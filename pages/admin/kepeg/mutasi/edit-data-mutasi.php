<div class="row">
	...
	$hasil	= mysqli_fetch_array($tampilMut);
	$id_peg	= $hasil['id_peg'];
	...
	if ($_POST['edit'] == "edit") {
		...
		if (empty($_POST['jns_mutasi']) || empty($_POST['tgl_mutasi']) || empty($_POST['no_mutasi'])) {
			...
			$emp_status = mysqli_query($koneksi, "UPDATE tb_pegawai SET status_mut='' WHERE pegawai_id='$id_peg'");

			$update = mysqli_query($koneksi, "UPDATE tb_mutasi SET jns_mutasi='$jns_mutasi', tgl_mutasi='$tgl_mutasi', no_mutasi='$no_mutasi' WHERE id_mutasi='$id_mutasi'");
