<div class="row">
	...
	$hasil	= mysqli_fetch_array($tampilPro);
	$notnip	= $hasil['nip'];
	...
	if ($_POST['edit'] == "edit") {
		...
		if (empty($_POST['nip']) || empty($_POST['nama']) || empty($_POST['tempat_lhr']) || empty($_POST['tgl_lhr']) || empty($_POST['agama']) || empty($_POST['jk']) || empty($_POST['gol_darah']) || empty($_POST['status_nikah']) || empty($_POST['status_kepeg']) || empty($_POST['tgl_naikpangkat']) || empty($_POST['tgl_naikgaji']) || empty($_POST['sisa_cuti'])) {
			...
			$update = mysqli_query($koneksi, "UPDATE tb_pegawai SET nip='$nip', nama='$nama', tempat_lhr='$tempat_lhr', tgl_lhr='$tgl_lhr', agama='$agama', jk='$jk', gol_darah='$gol_darah', status_nikah='$status_nikah', status_kepeg='$status_kepeg', tgl_naikpangkat='$tgl_naikpangkat', tgl_naikgaji='$tgl_naikgaji', alamat='$alamat', telp='$telp', email='$email', tgl_pensiun='$tgl_pensiun', unit_kerja='$id_unit', sisa_cuti='$sisa_cuti' WHERE id_peg='$id_peg'");
