...
// mencari tanggal 30 hari lalu dari tanggal terbaru
...
$tampilPres    = mysqli_query($koneksi, "SELECT * FROM att_log WHERE DATE(scan_date) >= '$scan_date2' AND DATE(scan_date) <= '$scan_date[scan_date]' AND pin = $peg[pegawai_pin] ORDER BY scan_date DESC");
    ...
    <div class="col-md-12">
        ...
        <div class="tab-content">
                    ...
                    <table id="" class="table table-bordered table-striped display">
                        ...
                        <tbody>
                            ...
                            if (!empty($_POST['periode_awal']) && !empty($_POST['periode_akhir'])) {
                                $tampilCari = mysqli_query($koneksi, "SELECT * FROM att_log WHERE DATE(scan_date) >= '$_POST[periode_awal]' AND DATE(scan_date) <= '$_POST[periode_akhir]' AND pin = '$peg[pegawai_pin]'");
                                $no = 0;
            ...
            <div class="tab-pane fade" id="rekapperiode">
                <div class="alert alert-success fade in">
                    <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span></button>
                    <i class="fa fa-info fa-2x pull-left"></i> Masukan tanggal awal bulan dan tanggal akhir bulan untuk melihat rekap periode ...
                </div>
                        ...
                        <tbody>
                            ...
                            if (!empty($_POST['periode_awal']) && !empty($_POST['periode_akhir'])) {
                                ...
                                if (($interval->m) > '0') {
                                    ...
                                    $tampilCari = mysqli_query($koneksi, "SELECT SUM(jk_count_as) AS hadir, SUM(late) AS terlambat FROM shift_result WHERE tgl_shift >= '$_POST[periode_awal]' AND tgl_shift <= '$_POST[periode_akhir]' AND pegawai_id = '$peg[pegawai_id]'");
                                    $cari = mysqli_fetch_array($tampilCari, MYSQLI_ASSOC);

                                    $jabatan    = mysqli_query($koneksi, "SELECT * FROM pembagian1 WHERE pembagian1_id='$peg[pembagian1_id]'");
                                    $jab    = mysqli_fetch_array($jabatan, MYSQLI_ASSOC);

                                    $queryIzin = mysqli_query($koneksi, "SELECT COUNT(izin_jenis_id) AS izin FROM shift_result WHERE tgl_shift >= '$_POST[periode_awal]' AND tgl_shift <= '$_POST[periode_akhir]' AND pegawai_id = '$peg[pegawai_id]' AND izin_jenis_id != '0'");
